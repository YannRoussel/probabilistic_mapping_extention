...
1. In the `data` folder we generated celltype-by-brain area matrices for better access to the densities. 
   - All files are .csv files. We show how to access them in notebook tutorials
   - e/m/me-types are kept separately as the naming of the files suggest
   - As for the brain regions: The naming follows the Allen Institute's nomenclature. You can choose to include unassigned regions if you choose the file which contains this in its file name. 
   - all celltypes means everything is put together in a large single file

2. In the `etype` folder you can find the generated figure inlets for all (ephys) "electric-types" both in the cross-section of a generated axial or transversal example brain slice. Find these in the folders `etype/fig_me_ax` for axial and `etype/fig_me_tr` for sagittal cross-section images. You can find notebooks how to generate and concatenate these for the figures. 
3. In the `figures` folder you can find all the m/e/me-type inlets from their corresponding folders concatenated into composite images. Note that the notebooks help you to generate these (including the brain slices as 2D-arrays in .npy file formats which are not included here).
4. In the `laminar_validation` folder you can find code to generate cell-type compositions plotted in different brain regions and the resulting images (in the `laminar_validation/results` folder). As an example we plot excitatory and inhibitory m/e/me-types in the AUD, VISp, and SSs layers of the CTX.  
5. In the `me-types` folder you can find the generated figure inlets for all "morpho-electric types" both in the cross-section of a generated axial or transversal example brain slice. Find these in the folders `me-types/fig_me_ax` for axial and `me-types/fig_me_tr` for sagittal cross-section images. You can find notebooks how to generate and concatenate these for the figures. 
6. In the `mtypes` folder you can find the generated figure inlets for all "morpho-electric types" both in the cross-section of a generated axial or transversal example brain slice. Find these in the folders `m-types/fig_me_ax` for axial and `m-types/fig_me_tr` for sagittal cross-section images. You can find notebooks how to generate and concatenate these for the figures.
7. In the `notebooks` folder contains a tutorial in Jupyter notebooks format. 
   - `compare_your_data.ipynb`: helps you to visualize any cell type in any brain slice together with you own data. This is the only notebook where we use `ipywidgets`. 
   - `create_all_celltype_df.ipynb`: this is the intermediate step on how to concatenate cell types into the all_celltypes_composition_with_unassigned_regions.csv file in the data folder. 
   - `e_type_slices_for_figure.ipynb`: this shows you how to generate .npy files / 2D-numpy arrays of brain slices with e-type densities. 
   - `laminarity_structure_metypes.ipynb`: This was the test code to show laminar organisation of cell types (me) for validation. It's easier to understand here what is going on in the Python code. 
   - `laminarity_structure_mtypes.ipynb`:  This was the test code to show laminar organisation of cell types (m) for validation.
   - `m_type_slices_for_figure.ipynb`:  this shows you how to generate .npy files / 2D-numpy arrays of brain slices with m-type densities. 
   - `me_type_slices_for_figure.ipynb`:  this shows you how to generate .npy files / 2D-numpy arrays of brain slices with me-type densities. 
   - `transcribing_t_types.ipynb`: a simplified version of `densities_app/transcribe_t_types_to_me_types.ipynb` where we only go through the necessary points and where I included how to save the .csv files in the data folder. 
   - `visualisation_multiproc.ipynb`:  a simplified version of `densities_app/visualisation_multiproc.ipynb` where we only go through the necessary points. 
   - `vizualize_celltypes_supp_figures.ipynb`: Notebook on how to generate some of the supplementary figures for the white paper. These composite images can be found in the `figures` folder. 
...
### Step 4 (Optional): Extension Folder
